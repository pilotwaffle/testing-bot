...
class SimpleNotificationManager:
    ...
    def __init__(self):
        ...
        self.last_error_notification_time: Dict[str, datetime] = {}
    ...
    async def notify(self, title: str, message: str, priority_str: str = "INFO"):
        ...
        if priority_level >= NotificationPriority.ERROR:
            cooldown_key = f"{title}:{message[:50]}"
    ...
    # --- Unified Method: send_notification ---
    async def send_notification(self, message: str, level: str = "INFO"):
        """Generic async notification method for compatibility with trading engine."""
        await self.notify(title="Notification", message=message, priority_str=level)
    ...
    def _get_discord_color(self, priority: str) -> int:
        colors = {
            "DEBUG": 0xAAAAAA, "INFO": 0x2ECC71, "WARNING": 0xF7D002, "ERROR": 0xFF6961,
            "HIGH": 0xE67E22, "CRITICAL": 0xE74C3C, "EMERGENCY": 0x9B59B6
        }
        return colors.get(priority.upper(), 0x3498DB)
