# ai/chat_manager.py - CORRECTED VERSION
...
from core.chat_bot import ChatBot # Ensure this import is correct based on your setup
...
class ConversationMemory:
    ...
    def set_trading_engine(self, trading_engine_instance):
        """Set the trading engine instance after it's initialized."""
        self.trading_engine = trading_engine_instance
        logger.info("🔗 Trading Engine instance set in Chat Manager.")
...
class EnhancedChatManager:
    ...
    async def _build_enhanced_context(self, intent: Intent = None) -> str:
        ...
        try:
            ...
            # REMOVED EMOJI TO AVOID UNICODE DECODING ERROR
            context_parts = [
            ...
            return "Trading Bot Assistant (Limited context available)" # Also removed emoji here
...
• Set your preferences with `/settings`"""
    ...
    async def _handle_portfolio_command(self, args: List[str]) -> str:
        ...
        try:
            status = await self._get_comprehensive_status()
            return f"📊 Your portfolio value is ${status.get('total_value', 0):.2f}. More details coming soon!"
        except Exception as e:
    ...
    async def _handle_positions_command(self, args: List[str]) -> str:
        ...
        try:
            ...
            return f"📈 You currently have {num_positions} open positions."
        except Exception as e:
    ...
    async def _handle_market_command(self, args: List[str]) -> str:
        ...
        # Placeholder implementation
        return "I'm gathering the latest market overview for you."
    ...
    async def _handle_strategies_command(self, args: List[str]) -> str:
        ...
        try:
            ...
            if active_strategies:
                return f"⚡ Active strategies: {', '.join(active_strategies)}. Use /strategies [name] to see details."
            else:
                return "No strategies are currently active. Use /strategies activate [name] to enable one."
        except Exception as e:
    ...
    async def _handle_risk_command(self, args: List[str]) -> str:
        ...
        try:
            status = await self._get_comprehensive_status()
            return f"🎯 Current risk level: {status.get('risk_level', 'Unknown')}/10. Max drawdown: {status.get('max_drawdown', 'N/A')}."
        except Exception as e:
            return f"Error fetching risk data: {e}"

    async def _handle_settings_command(self, args: List[str]) -> str:
        """Handle settings command"""
        # Placeholder implementation
        return "⚙️ Settings management is under development. You can set communication style, risk tolerance, etc."

    async def _handle_history_command(self, args: List[str]) -> str:
        """Handle history command"""
        # Placeholder implementation
        history_summary = self.memory.create_summary()
    ...
    async def _handle_help_command(self, args: List[str]) -> str:
        ...
        - `/portfolio`: Get an overview of your current portfolio performance.
        - `/positions`: View all your open trading positions.
        - `/market [symbol] [timeframe]`: Get detailed market analysis for a symbol (e.g., `/market BTC/USD 1h`).
        - `/trade [buy/sell] [symbol] [amount] [price]`: Place a trade order (e.g., `/trade buy BTC/USD 0.001 60000`).
        - `/cancel [order_id]`: Cancel a pending order.
        - `/strategies [list/activate/deactivate] [name]`: Manage trading strategies.
        - `/risk`: Get current risk assessment and metrics.
        - `/settings`: Access bot settings and preferences.
        - `/history`: Summarize your conversation history.
