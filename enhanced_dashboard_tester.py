...
import requests
print("--- Imported requests ---") # Diagnostic print
import json
print("--- Imported json ---")# Diagnostic print
import time
print("--- Imported time ---")   # Diagnostic print
import os
print("--- Imported os ---")     # Diagnostic print
from pathlib import Path
print("--- Imported Path ---")   # Diagnostic print
from concurrent.futures import ThreadPoolExecutor, as_completed
print("--- Imported ThreadPoolExecutor and as_completed ---") # Diagnostic print
...
print(f"--- PROJECT_ROOT calculated: {PROJECT_ROOT} ---") # Diagnostic print
...
# Diagnostic prints for path existence
...
print(f"DASHBOARD_HTML_PATH: {DASHBOARD_HTML_PATH} (Exists: {DASHBOARD_HTML_PATH.exists()})")
print(f"STYLE_CSS_PATH: {STYLE_CSS_PATH} (Exists: {STYLE_CSS_PATH.exists()})")
print(f"ENHANCED_DASHBOARD_JS_PATH: {ENHANCED_DASHBOARD_JS_PATH} (Exists: {ENHANCED_DASHBOARD_JS_PATH.exists()})")
print(f"MAIN_PY_PATH: {MAIN_PY_PATH} (Exists: {MAIN_PY_PATH.exists()})")
...
def test_get_endpoint(path, expected_status=200):
    ...
    try:
        ...
        if response.status_code == expected_status:
            ...
            test_results["api_tests"].append({"endpoint": path, "method": "GET", "status": "FAIL", "details": f"Expected {expected_status}, Got {response.status_code}"})
...
def run_tests():
    ...
    try:
        ...
        print(f"--- ERROR during file validation: {e} ---") # Catch all for file validation
    ...
    health_avg_time = float(test_results.get("performance_tests", {}).get("/health", {}).get("avg_ms", "0"))
...
# --- SCRIPT ENTRY POINT ---
# This block ensures the tests run only when the script is executed directly.
