...
def _str_to_float_or_unlimited(s: Optional[str], default: Union[float, str]) -> Union[float, str]:
    if s is None:
        return default
    ...
    try:
        ...
        logging.warning(f"Invalid stake_amount value '{s}', using default '{default}'")
        return default
...
class NotificationConfig:
    ...
    # DISABLED_NOTIFICATION: slack_webhook: str = ""
    # Discord
    discord_enabled: bool = False
    # DISABLED_NOTIFICATION: discord_webhook: str = ""
    # Email
...
class ConfigManager:
    ...
    def _create_default_config(self) -> EnhancedSettings:
        ...
        config.trading.stake_amount = _str_to_float_or_unlimited(os.getenv('STAKE_AMOUNT'), 100.0)
        ...
        for env_key, env_value in os.environ.items():
            for prefix in prefixes:
                if env_key.startswith(prefix):
                    ...
                    logging.debug(f"Applied environment override: {env_key}")  # Lowered to debug level
                    break
        return config
    ...
    def _dict_to_settings(self, config_dict: Dict[str, Any]) -> EnhancedSettings:
        ...
        if 'trading' in config_dict:
            ...
            # Ensure 'stake_amount' is float or "unlimited"
            if 'stake_amount' in trading_dict:
                sa = trading_dict['stake_amount']
                trading_dict['stake_amount'] = _str_to_float_or_unlimited(sa, 100.0)
            settings.trading = TradingConfig(**trading_dict)
    ...
    def validate_config(self) -> List[str]:
        ...
        sa = self.config.trading.stake_amount
        if isinstance(sa, (int, float)) and sa <= 0:
            issues.append("stake_amount must be greater than 0 or 'unlimited'")
        elif isinstance(sa, str) and sa.lower() != "unlimited":
            issues.append("stake_amount string value must be 'unlimited' or a positive number")
