...
_connection_manager = None # Add this for the WebSocket ConnectionManager
...
def get_trading_engine_dep():
    if _trading_engine is None:
        # This should ideally not happen if app starts correctly
...
def get_gemini_ai_dep() -> Optional[object]: # Use object as base type for optional
    # Can be None if AI is not enabled or failed to initialize
    return _gemini_ai
...
# You might also want a helper to set these from main.py's lifespan
# This avoids directly manipulating _globals from outside this module
def set_dependencies(
    trading_engine_instance, chat_bot_instance, gemini_ai_instance,
    notification_manager_instance, connection_manager_instance
):
    global _trading_engine, _chat_bot, _gemini_ai, _notification_manager, _connection_manager
